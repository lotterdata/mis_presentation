...
```{r,results='asis'}
pa3 <- lm(prize_3~I(n1+n2+n3+n4+n5)*draw.since.jp,data=pa.train)
knitr::kable(summary(pa3)$coefficients,digits=4)
print(summary(pa3)$r.squared)
```
...
pa.train$hitfact <- cut(pa.train$draw.since.jp,
                     cutpoints,
                     include.lowest=TRUE,
                     right = FALSE)

g <- ggplot(pa.train,aes(draw_sum,prize_3,col=hitfact))
