...
```{r}
tsjp <- split(pa.train,pa.train$jpfactor)
...
```{r}
nj.test$drawdate <- as.Date(as.character(nj.test$drawdate))

draw.since.jp.nj <- function(x) {
    jp.dates <- filter(nj.test, prize_5 > 0 & drawdate < x) %>%
                arrange(desc(drawdate))
                    
    return(nrow(filter(nj.test,
                       drawdate > jp.dates$drawdate[1] & 
                       drawdate <= x)))
}

nj.test$draw.since.jp <- sapply(nj.test$drawdate,draw.since.jp.nj)

nj.test<- filter(nj.test, drawdate > 
                       min(nj.test$drawdate[nj.test$prize_5>0]))
```
